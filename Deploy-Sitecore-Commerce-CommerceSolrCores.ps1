#Requires -Version 3
param(
    [string]$SiteName = "SITECORE",	
	[string]$SiteHostHeaderName = "SITECOREuatcm.SITECOREuat.loc",	
...
$params = @{
        Path = Resolve-Path '.\Configuration\Commerce\Master_CommerceSolrCores.json'	
		SiteName = $SiteName
		SiteHostHeaderName = $SiteHostHeaderName 
		InstallDir = "C:\inetpub\SITECORE.sc\Website"
		XConnectInstallDir = "c:\inetpub\wwwroot\xp0.xconnect"
		CertificateName = $SiteName
		CommerceServicesDbServer = $($Env:COMPUTERNAME)
		CommerceServicesDbName = "SitecoreCommerce9_Shared"
		...
		SitecoreUserPassword = "rVRkMQ5yBN3T0TH"
		CommerceSearchProvider = $CommerceSearchProvider
		SolrUrl = "https://SITECOREuatindex.SITECOREuat.loc/solr"
		SolrRoot = "C:\\Program Files\solr66"
		SolrService = "solrservice"
		SolrSchemas = ( Join-Path -Path $DEPLOYMENT_DIRECTORY -ChildPath "SolrSchemas" )
		SearchIndexPrefix = $Prefix
		AzureSearchServiceName = ""
		AzureSearchAdminKey = ""
		AzureSearchQueryKey = ""
				CommerceEngineDacPac = Resolve-Path -Path "C:\Sitecore.Commerce.2018.07-2.2.126\Sitecore.Commerce.Engine.SDK.2.2.72\Sitecore.Commerce.Engine.DB.dacpac"	   
		...
		SitecoreCommerceEngineZipPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore.Commerce.Engine.2.2.126.zip"		
		SitecoreBizFxServicesContentPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore.BizFX.1.2.19"		
		SitecoreIdentityServerZipPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore.IdentityServer.1.2.3.zip"
		CommerceEngineCertificatePath = Resolve-Path -Path "C:\deploy_xc\sx9u2.engineconnect.cer"		
        SiteUtilitiesSrc = ( Join-Path -Path $DEPLOYMENT_DIRECTORY -ChildPath "SiteUtilityPages" )	
        HabitatImagesModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore.Commerce.Habitat.Images-1.0.0.zip"	
        AdvImagesModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Adventure Works Images.zip"	
        CommerceConnectModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore Commerce Connect Core 11.2.83.zip"	
        CEConnectPackageFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore.Commerce.Engine.Connect.2.2.86.update"
        PowerShellExtensionsModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore PowerShell Extensions-4.7.2 for Sitecore 8.zip"
        SXAModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore Experience Accelerator 1.8 rev. 181112 for 9.0.zip"
        SXACommerceModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore Commerce Experience Accelerator 1.2.205.zip"
		SXAStorefrontModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore Commerce Experience Accelerator Storefront 1.2.205.zip"
        SXAStorefrontThemeModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore Commerce Experience Accelerator Storefront Themes 1.2.205.zip"
		SXAStorefrontCatalogModuleFullPath = Resolve-Path -Path "c:\Sitecore.Commerce.2018.07-2.2.126\Sitecore Commerce Experience Accelerator Habitat Catalog 1.2.205.zip"
		MergeToolFullPath = Resolve-Path -Path "C:\Program Files\WindowsPowerShell\Modules\SitecoreInstallFramework\1.2.1\lib\Microsoft.Web.XmlTransform.dll"
		UserAccount = @{
			Domain = 'SITECOREUAT'
			UserName = 'ianw'
